...
include('../crearConexionVam.php');
 ...
 if ($estado=='A') {
   echo "<script>";
    ...
    if ($estado=='I') {
      echo "<script>";
       echo "window.location = 'Manual.php?x=".$_GET['x']." ';"; 
       echo "</script>";
      }
 ?>
<!DOCTYPE html>
...
if ($row=mssql_fetch_array($ConsultaNombre)) {
    ...
    $Dia_Final_AC=(date('d',strtotime($date_future1)));
     ...
     $FechaF_Acuerdo= fecha($Dia_Final_AC,$mes_Final_AC,$anio_Final_AC);
      ...
      $mensaje1="por la modalidad de Contrato en el periodo comprendido del ".$FechaincioAcuerdo." al ".$FechaF_Acuerdo." y Acuerdo el ".$FechaIncioContrato." al ".$FechaFinalAcuerdo;
    }

     if (strtotime($fechaIncioContrato) == strtotime($fechaInicioAcuerdo)) {
      $mensaje1="por la modalidad de Acuerdo el ".$FechaincioAcuerdo." al ".$FechaFinalAcuerdo;
    }
